...
# for some reason you need python@3.8 and pipenv on separate lines
.PHONY: pipenv-python
pipenv-python:	pipenv-clean
	...
	[[ ! -e .env ]] && echo "PYTHONPATH=${PWD}" > .env || true
...
# https://stackoverflow.com/questions/38801796/makefile-set-if-variable-is-empty/38832692
# Shell hackery that [ $var ] returns presence
...
lint:
	...
	pipenv run flake8
	[ $(all_py) ] && pipenv run mypy --namespace-packages $(all_py) || true
	[ $(all_py) ] && pipenv run bandit $(all_py) || true
	[ $(all_py) ] && pipenv run pydocstyle --convention=google $(all_py) || true
	# lint the yaml config files and kill the error if it doesn't exist
	[ $(all_yaml) ] && pipenv run yamllint $(all_yaml) || true
