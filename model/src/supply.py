...
class Essential():
    ...
    The first is a cost matrix for essential level what is their cost for each of n
    itemrs or e x n 
    ...
    def __init__(self, model):
        self.cost_per_resource_values = np.array([[3, 0.5],
                                                  [4.5, 0.75]])

        self.cost_per_resource_en_df = pd.DataFrame(
                self.cost_per_resource_arrays,
                index=model.essential.labels,
                columns=model.resource.labels)
