...
class PopulationOES(LoaderCSV):
    ...
    def __init__(self, oes_path: str = OES_PATH, code_path: str = CODE_PATH,
                 pop_path: str = POP_PATH):

        df_list = []
        super().__init__(oes_path, code_path, pop_path)

        for fname in self.p_list:
            df_list.append(self.load_df(fname))

        # Format the OES dataframe
        df_list[0] = self.format_oes(df_list[0])

        # Format the MSA code dataframe
